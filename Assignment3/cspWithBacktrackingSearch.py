...
def BackTrackingSearch(list1,list2) :
    return RecursiveBackTrackingSearch(list1,list2)
...
4. After assigning we call the function again
5. If any consistency fails we remove the assignment and check for other variables and TA nodes   
"""
def RecursiveBackTrackingSearch(list1,list2) :
    if checkCompleteAssignment(list1) :
        return list1
    var = selectUnassignedVariable(list1)    
    for value in orderDomainValues(var,list2) :
        if checkConsistent(value,var,list1) :
            assignValue(var,value)
            result = RecursiveBackTrackingSearch(list1,list2)
            if result != "Fail" :
                return result
...
def checkCompleteAssignment(list) :
    for item in list :
        if (len(item.taAssigned) == 0) or (item.tasRequired > 0) :
...
def checkConsistent(value,var,list) :
...
"""
The function splits the given course list into there lists :
1. TAreqList : The courses list which requires TA assignments
2. NoTAreqList : The courses list which doesn't require TA assignments 
3. DummyTAList : The courses list which doesn't match any skills for the given TA skills
"""
...
result1 = BackTrackingSearch(TAreqList,TADomainList)

"""
Printing the Assignments for courses
"""
