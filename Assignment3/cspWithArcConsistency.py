...
"""
The main logic of backtracking is done here , initially we pass the course Lists and TA lists
...
for item1 in TAreqList :
    for item2 in TAreqList :
        if not (item1.courseName == item2.courseName) :
            queue.put([item1,item2])

def checkOtherDomains(value,list1,list2 ) :
    if value == 0.5 :
        for item in list2 :
            if item.taName == list1[0].taName :
                if item.taCapacity > 0.5 :
                    return True
            else :
                return True
        return False
    if value == 1.5 :
        ...
        secondTA = list1[1]
        if len(list2) == 1 :
            if (list2[0].taName != firstTA.taName) and (list2[0].taName != secondTA.taName) :
                return True
            if (list2[0].taName != firstTA.taName) and (list2[0].taName == secondTA.taName) :
                if item.taCapacity > 0.5 :
                    return True
            return False
        valueList = [list1[0].taName,list1[1].taName]
        checkList=[]
        for item in list2 :
            checkList.append(item.taName)
        checkListlen = len(checkList)
        interList = set(valueList).intersection(checkList)
        if (checkListlen - len(interList)) >= 0 :
            return True
        return False 
    if value == 2 :
        valueList = [list1[0].taName,list1[1].taName]
        ...
        checkListlen = len(checkList)
        interList = set(valueList).intersection(checkList)
        if (checkListlen - len(interList)) >= 1 :
            return True
        return False         
                           
def deleteValue(value,list1,list2):
    if value == 0.5 :
        for item in list2 :
            if item.taName == list1[0].taName :
                item.taCapacity == item.taCapacity - 0.5
    if value == 1.5 :
        for item in list2 :
            if item.taName == list1[0].taName :
                item.taCapacity == item.taCapacity - 1
        for item in list2 :
            if item.taName == list1[1].taName :
                item.taCapacity == item.taCapacity - 0.5
    if value == 2 :
        for item in list2 :
            if item.taName == list1[0].taName :
...
def RMInconsistentValues(pair) :
    courseone = pair[0]
    coursetwo = pair[1]
    removed = False
    if courseone.tasRequired == 0.5 :
        for value in courseone.domainList :
            if not value.taCapacity == 0 :
                if not checkOtherDomains(0.5,[value],coursetwo.domainList) :
                    deleteValue(0.5,[value],courseone.domainList)
                    removed = True
                    return removed
    if courseone.tasRequired == 1.5 :
        for item1 in courseone.domainList :
            if item1.taCapacity == 1 :
                for item2 in courseone.domainList :
                    if not item1.taCapacity == 0 :
                        if not checkOtherDomains(1.5,[item1,item2], coursetwo.domainList) : 
                            deleteValue(1.5,[item1,item2],courseone.domainList)
                            removed = True
                            return removed
    if courseone.tasRequired == 2 :
        for item1 in courseone.domainList :
            if item1.taCapacity == 1 :
                for item2 in courseone.domainList :
                    if item2.taCapacity == 1 :
                        if not checkOtherDomains( 2,[item1,item2], coursetwo.domainList) :
                            deleteValue(2,[item1,item2],courseone.domainList)
                            removed = True
                            return removed
    return removed


"""The function takes course list and queue containing the total arcs """
def ArcConsistency(list1,queue1) :
    while not queue1.empty() :
        coursePair = queue1.get()
        if RMInconsistentValues(coursePair) :
            for item in list1 :
                if not (item.courseName == coursePair[0].courseName) :
                    queue1.put([item,coursePair[0]])
    return list1                
...
ArcConsistency(TAreqList, queue)
